...
const Protected: React.FC<ProtectedProps> = ({ setIsError, setResponseError }) => {
    ...
    return (
        <>
                        ...
                        <div className="flex gap-2 justify-center">
                            {[...Array(5)].map((_, index) => (
                                <input
                                    ...
                                    style={{ borderColor: code[index] ? '#3f9f75' : "#9f3f3f" }}
                                />
                            ))}
                        </div>

                        <div className="flex items-center justify-center gap-3 mt-[1.5rem] text-[14px]">
                            <p className="cursor-pointer hover:text-[#fff]">Resend Code</p>
                            <p className="bg-[#3c3d488f] p-1 px-[0.7rem] rounded-md cursor-pointer hover:text-[#fff]" onClick={() => {
                                try {
            ...
            <div className="flex items-center gap-3 cursor-pointer fixed bottom-7 left-[3rem] bg-[#333333a6] p-[0.4rem] px-[0.6rem] rounded opacity-[0.4] hover:opacity-[1] transition" onClick={() => setIsModal(true)}>
                ...
                <div className="flex flex-col gap-5 justify-center h-[95vh] items-center">
                    ...
                    <div className="flex flex-col justify-center gap-6 bg-[#1b1b22e0] min-w-[27rem] min-h-[15rem] border border-[#444] rounded-md text-center">
                        ...
                        {paswd == '' ? (
                            ...
                            <p className="text-[1.7rem] tracking-[0.5rem] font-medium">{paswd}</p>
                        )}

                        <div className="flex items-center justify-center gap-3 mt-[1.5rem] text-[14px]">
                            <p onClick={() => setIsModal(false)} className="cursor-pointer hover:text-[#fff]">Back</p>
                            <p className="bg-[#3c3d488f] p-1 px-[0.7rem] rounded-md cursor-pointer hover:text-[#fff]" onClick={() => {
