...
describe('Global <Index /> testing...', () => {
	...
	it('Show base monitoring', async () => {
		...
		expect(container.querySelector('.server__load__memory')?.textContent).toEqual('70%');
		expect(container.querySelector('.server__load__cpu')?.textContent).toEqual('50.00%');
		expect(container.querySelector('.server__load__network')?.textContent).toEqual('2.00%');
	});
});
